...
class ApiClient {
  ...
  async getAdminModelsAndConfigs() {
    return this.request<any>("/bff/admin/models", {}, true);
  }
  /**
   * [MỚI] Lấy danh sách tất cả AI model và cấu hình đi kèm của chúng.
   */
  async getAdminModelsAndConfigs() {
    return this.request<any>("/bff/admin/models", {}, true);
  }
  ...
  /**
   * [MỚI] Kích hoạt một model bằng ID của nó.
   */
  async activateAdminAIModel(modelId: string) {
    return this.request<any>(
      `/bff/admin/models/${modelId}/activate`,
      { method: "POST" },
      true
    );
  }
  ...
  /**
   * [MỚI] Cập nhật các tham số của một cấu hình bằng ID của config.
   */
  async updateAdminAIConfig(configId: string, params: any) {
    return this.request<any>(
      `/bff/admin/configs/${configId}`,
      {
        method: "PUT",
  ...
  /**
   * [MỚI] Upload một model mới lên hệ thống.
   */
  async uploadAdminAIModel(formData: FormData) {
    return this.requestWithFormData<any>(
  ...
  // THÊM MỚI: Lấy chi tiết một Plan cho Admin bằng ID
  async getAdminPlanById(planId: string) {
    return this.request<any>(
