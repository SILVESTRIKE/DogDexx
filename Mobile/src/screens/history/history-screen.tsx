...
export default function HistoryScreen({navigation, route}:Props): React.JSX.Element {
  const { user } = useAuth();
  const { t } = useI18n();
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      Toast.show({
        type: 'success',
        text1: result.message || t('history.deleteSuccess'),
      });
    } catch (error: any) {
  ...
  const handleDownload = async (prediction: PredictionHistoryItem) => {
    ...
    try {
      Toast.show({
        type: 'info',
        text1: 'Preparing file for download...',
      });
      ...
      Toast.show({
        type: 'success',
        text1: t('history.downloadSuccess'),
      });
    } catch (error) {
      console.error('Download failed:', error);
      Toast.show({
        type: 'error',
        text1: t('history.downloadError'),
      });
    }
  };
