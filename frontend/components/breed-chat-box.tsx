...
function SimpleMarkdownRenderer({ text }: { text: string }) {
  ...
  return (
    <div className="space-y-2">
      {lines.map((line, i) => {
        if (line.startsWith("### ")) return <h3 key={i} className="text-md font-semibold mt-3 mb-1">{line.substring(4)}</h3>
        if (line.startsWith("## ")) return <h2 key={i} className="text-lg font-bold mt-3 mb-1">{line.substring(3)}</h2>
        if (line.startsWith("# ")) return <h1 key={i} className="text-xl font-extrabold mt-3 mb-1">{line.substring(2)}</h1>
    ...
    <>
      {!isOpen && (
        <motion.div
          ...
          style={{ position: "fixed", top: 0, left: 0, zIndex: 50, cursor: "grab" }}
          className="select-none"
        >
          <Button
            className={cn( "border", "select-none",
              "shadow-lg transition-all duration-300 h-auto",
              {
                'rounded-l-lg rounded-r-none px-4 py-3 flex items-center': snapEdge === 'right',
                'rounded-r-lg rounded-l-none px-4 py-3 flex items-center': snapEdge === 'left',
                'rounded-full w-14 h-14 p-0 justify-center': snapEdge === 'none'
              }
            )}
            onClickCapture={(e) => e.stopPropagation()}
          >
            <img
              src="/LogoWebWhite.png" alt="Chat AI"
              className={cn({'mr-2 w-5 h-auto': snapEdge !== 'none', 'w-6 h-6': snapEdge === 'none'})}
            />
            ...
            <Card className="w-full max-w-sm shadow-2xl border-2 border-chart-2 cursor-default">
              <CardHeader className="flex flex-row items-center justify-between p-4 border-b">
                <CardTitle className="flex items-center gap-2 text-lg">
                  <img src="/LogoWebWhite.png" alt="Chat AI" className="w-5 h-auto" />
                  ...
                  <div className="space-y-4">
                    ...
                    {messages.map((message, index) => (
                      <div key={index} className={cn("flex gap-3 text-sm", message.role === "user" ? "justify-end" : "justify-start")}>
                        {message.role === "model" && (
                          <Avatar className="h-8 w-8"><AvatarImage src="/LogoWebWhite.png" alt="AI" /></Avatar>
                        )}
                        ...
                        {message.role === "user" && (
                          <Avatar className="h-8 w-8">
                            ...
                            <AvatarFallback>{user?.username ? user.username.charAt(0).toUpperCase() : <User className="h-5 w-5" />}</AvatarFallback>
                          </Avatar>
                        )}
                      </div>
                    ))}
                    ...
                    {messages.length <= 1 && !isLoading && (
                      <div className="pt-4 border-t">
                        <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1.5 justify-center"><Sparkles className="w-3 h-3" /> Gợi ý cho bạn:</p>
                        <div className="grid grid-cols-2 gap-2">
                          {samplePrompts.map((prompt, i) => (
                            <Button key={i} variant="outline" size="sm" className="text-xs h-auto py-1.5 whitespace-normal text-left justify-start" onClick={() => submitMessage(prompt)}>
                              {prompt}
                            </Button>
                          ))}
                        </div>
