...
export class AdminService {
  ...
  async deleteMedia(mediaId: string): Promise<{ message: string }> {
    ...
    try {
      ...
      console.log(`[Admin Service] Deleting Cloudinary resource for media ${mediaId}: ${public_id}`);
      await cloudinary.uploader.destroy(public_id);
    } catch (error: any) {
      if (error.http_code !== 404) console.error(`Không thể xóa file trên Cloudinary: ${media.mediaPath}`, error.message);
    }
  ...
  async browseDatasetDirectory(relativePath: string): Promise<{ directories: DirectoryItem[], files: any[] }> {
    try {
      ...
      const folderPath = relativePath.startsWith('/') ? relativePath.substring(1) : relativePath;
      ...
      // Map kết quả thư mục
      const directories: DirectoryItem[] = folderResult.folders.map((folder: { name: string; path: string; }) => ({
        id: folder.path,
        name: folder.name,
        type: 'folder',
      }));
      ...
      const files = resourcesResult.resources
        .filter((resource: { public_id: string; }) => {
            ...
            const pathWithoutPrefix = resource.public_id.substring(folderPath.length ? folderPath.length + 1 : 0);
            return !pathWithoutPrefix.includes('/');
        })
        .map((resource: { public_id: string; format: string; resource_type: string; bytes: number; created_at: string; secure_url: string; }) => ({
            id: resource.public_id,
            name: path.basename(resource.public_id) + (resource.format ? `.${resource.format}` : ''),
      ...
      console.error(`[Admin Service] Error browsing Cloudinary path '${relativePath}':`, error);
      // Xử lý lỗi từ Cloudinary, ví dụ thư mục không tồn tại
      if (error.http_code === 404) {
        throw new NotFoundError(`Directory not found on Cloudinary: ${relativePath}`);
      }
  ...
  async generateDatasetArchiveUrl(): Promise<string> {
    console.log('[Admin Service] Generating Cloudinary archive URL for "dataset" folder...');
    ...
    console.log('[Admin Service] Generated signed URL for dataset archive.');
    return url;
  }
  /**
