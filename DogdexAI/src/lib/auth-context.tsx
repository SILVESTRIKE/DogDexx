...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const handleTokenUpdate = useCallback(
    (tokens: { remaining: number; limit: number }) => {
      setUser((currentUser) => {
        ...
        if (
          currentUser &&
          (currentUser.remainingTokens !== tokens.remaining ||
            currentUser.tokenAllotment !== tokens.limit)
        ) {
          return {
            ...currentUser,
  ...
  const checkAuthStatus = useCallback(async () => {
    ...
    if (!await TokenManager.getAccessToken()) {
      try {
        ...
        console.log("Guest session response:", response);
        if (response.isGuest) {
          setUser({
            id: "guest-session",
            username: "Guest",
    ...
    try {
      ...
      // toast({
      //   title: t("auth.sessionExpiredTitle") || "Phiên đăng nhập đã hết hạn",
      //   description: t("auth.sessionExpiredMessage") || "Vui lòng đăng nhập lại để tiếp tục.",
      // });
      Alert.alert(
        t("auth.sessionExpiredTitle") || "Session Expired",
        t("auth.sessionExpiredMessage") || "Please log in again to continue."
      );
     await TokenManager.clearTokens();
      setUser(null);
    } finally {
  ...
  const register = async (data: any) => {
    ...
    const slugify = (str: string) =>
      str
        ...
        .replace(/\s+/g, "-")
        .replace(/[^\w-]+/g, "");
    const sanitizedUsername = slugify(data.username);
    if (!usernameRegex.test(sanitizedUsername)) {
      throw new Error(
        t("auth.errorUsernameInvalid") ||
          "Username must be lowercase, no spaces, no accents, and can only contain letters, numbers, and underscores (_)."
      );
    }
  ...
  const logout = useCallback(async () => {
    ...
    await  TokenManager.clearTokens();
      ...
      // toast({
      //   title: t("nav.logout"),
      //   description: t("auth.logoutSuccess") || "Bạn đã đăng xuất thành công.",
      // });
      Alert.alert(
        t("nav.logout") || "Logout",
        t("auth.logoutSuccess") || "You have been logged out successfully."
      );
  ...
  const verifyOtp = async (email: string, otp: string) => {
    ...
    Alert.alert(t("common.success") || "Success", response.message);
    return response;
  };
  ...
  const deletePredictionHistory = useCallback(
    async (historyId: string) => {
      ...
      Alert.alert(
        ...
        t("history.deleteSuccess") || "Prediction history deleted."
      );
    },
    [t]
  );
  ...
  const deleteAccount = useCallback(async (password: string) => {
    try {
     ...
     await TokenManager.clearTokens();
      setUser(null);
      // toast({
      //   title: t("account.deletedTitle") || "Account deleted",
      //   description:
      //     t("account.deletedMessage") || "Your account has been deleted.",
      // });
      Alert.alert(
        t("account.deletedTitle") || "Account Deleted",
        t("account.deletedMessage") || "Your account has been deleted."
      );
      await refetchUser();
    } catch (err) {
